      ...
      contents: [
        {
          ...
          parts: [
            {
              text: `List every food ingredient visible in this image.
Return only JSON in this exact shape:
{
  "ingredients": ["ingredient1", "ingredient2"]
}
Be conservative and only include likely visible edible ingredients.`,
            },
            {
              inlineData: {
                mimeType: file.type,
    ...
    setPhotoIngs(Array.isArray(parsed.ingredients) ? parsed.ingredients : []);
  } catch (err) {
    console.error(err);
    setPhotoIngs([]);
  } finally {
...
const fetchRecipes = async () => {
  if (!ingredients.length) {
    setErr("Round up some ingredients first, partner!");
    return;
  }
  ...
  const prefs = [
    ...
    tastes.length ? `Taste: ${tastes.join(", ")}` : "",
    meal ? `Meal type: ${meal}` : "",
    notes ? `Extra: ${notes}` : "",
  ]
    .filter(Boolean)
...
Generate exactly 4 varied recipes.`;

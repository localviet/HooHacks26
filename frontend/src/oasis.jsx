...
const G = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── Saloon door overlay (absolute, not fixed) ── */
    .saloon-wrap {
      position: absolute;
    ...
    .saloon-curtain {
      ...
      inset: 0;
      background: #0e0600;
      animation: curtainFade 2.8s ease forwards;
    }
    ...
    .sd::after {
      ...
      background: repeating-linear-gradient(92deg,
        ...
        rgba(0,0,0,0.13) 22px, rgba(0,0,0,0.13) 24px);
    }
    .sd-l {
      border-right: 5px solid #8b5a1a;
      transform-origin: left center;
      animation: swingL 1.6s cubic-bezier(0.25,0.46,0.45,0.94) 0.9s both;
    }
    .sd-r {
      border-left: 5px solid #8b5a1a;
    ...
    .door-plank {
      height: 5px; background: #0e0600;
      margin: 0 14px; border-radius: 2px;
      position: relative; z-index: 1;
    }
    .door-arch {
      width: 48%; aspect-ratio: 1 / 1.3;
    ...
    .door-arch-inner {
      width: 78%; height: 78%;
      border: 2px solid rgba(139,90,26,0.4);
      border-radius: 50% 50% 0 0;
    }
    .hinge {
      width: 14px; height: 28px;
      background: linear-gradient(90deg, #6b5010, #c9922a, #6b5010);
    ...
    .wanted-card::before { content:''; position:absolute; inset:6px; border:1px solid rgba(139,94,26,0.4); pointer-events:none; }
    ...
    .draw-btn { font-family:'Rye',cursive; font-size:18px; letter-spacing:0.08em; padding:16px 40px; background:linear-gradient(180deg,#c9501a 0%,#8b2800 100%); color:#f5e6cc; border:3px solid #f0c060; box-shadow:0 4px 0 #4a1200, 0 6px 20px rgba(0,0,0,0.5); cursor:pointer; transition:all 0.15s; position:relative; overflow:hidden; }
...
const CUISINES   = ["American","Mexican","Italian","Chinese","Japanese","Korean","Thai","Indian","Mediterranean","French","Southern","Cajun","BBQ"];
const TASTES     = ["Spicy","Savory","Sweet","Smoky","Tangy","Hearty","Light & Fresh","Comfort Food","Rich & Creamy","Crispy"];
const MEAL_TYPES = ["Breakfast","Lunch","Dinner","Snack","Dessert","Soup / Stew","Salad","Sandwich"];
...
/* ── Saloon Doors (uses absolute positioning) ── */
const Saloon = ({ onDone }) => {
...
const RecipeCard = ({ r, idx }) => {
  ...
  return (
    <div className="wanted-card fu" style={{animationDelay:`${idx*110}ms`,padding:"24px",color:"#3d1f05"}}>
      <div style={{textAlign:"center",marginBottom:12}}>
        ...
        <h3 className="rye" style={{fontSize:19,color:"#3d1205",lineHeight:1.2,marginBottom:4}}>{r.name}</h3>
        <p style={{fontSize:12,fontStyle:"italic",color:"#7a4a20"}}>{r.tagline}</p>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderTop:"1px solid #c9922a",borderBottom:"1px solid #c9922a",marginBottom:12}}>
      ...
      <p style={{fontSize:13,lineHeight:1.7,color:"#4a2810",marginBottom:12}}>{r.description}</p>
      <div style={{marginBottom:12}}>
        <p className="sl">You'll Need</p>
        <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
          {r.ingredients?.map(ing=>(
            <span key={ing} className="cp" style={{fontSize:11,background:"rgba(139,90,26,0.15)",border:"1px solid rgba(139,90,26,0.3)",padding:"2px 8px",color:"#5a3010"}}>{ing}</span>
          ))}
        </div>
      </div>
      <button onClick={()=>setOpen(o=>!o)} style={{width:"100%",background:"#8b5e1a",border:"1px solid #c9922a",color:"#f5e6cc",fontFamily:"Rye,cursive",fontSize:13,padding:"8px",cursor:"pointer",letterSpacing:"0.06em"}}>
        {open?"▲ Hide Steps":"▼ Show Steps"}
      </button>
      {open&&(
        <div style={{marginTop:12,paddingTop:12,borderTop:"1px dashed #c9922a"}}>
          <p className="sl">Method</p>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {r.steps?.map((s,i)=>(
              <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                <div className="step-num">{i+1}</div>
                <p style={{fontSize:13,lineHeight:1.65,flex:1}}>{s}</p>
              </div>
            ))}
          </div>
          {r.tip&&(
            <div style={{marginTop:12,padding:"10px 14px",background:"rgba(139,90,26,0.12)",border:"1px solid rgba(139,90,26,0.3)",borderLeft:"4px solid #8b5e1a"}}>
...
export default function Oasis() {
  ...
  const handleFile = useCallback(async file=>{
    ...
    try {
      ...
      const resp=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:file.type,data:b64}},{type:"text",text:'List every food ingredient visible. Return ONLY a JSON array of strings. Example: ["chicken","garlic"]'}]}]})});
  ...
  const fetchRecipes=async()=>{
    ...
    const prefs=[cuisines.length?`Cuisine: ${cuisines.join(", ")}`:"",tastes.length?`Taste: ${tastes.join(", ")}`:"",meal?`Meal type: ${meal}`:"",notes?`Extra: ${notes}`:""].filter(Boolean).join(". ");
    const prompt=`You are a world-class chef with deep expertise in authentic global cuisines.

The user has ONLY these ingredients available: ${ingredients.join(", ")}.

STRICT RULES:
1. Every ingredient in each recipe MUST come ONLY from the available list. Do NOT add anything not on that list — not salt, oil, water, spices, or any staple unless explicitly listed.
2. You do not need to use every ingredient. Pick whichever subset makes the best dish.
3. If a cuisine is requested, make it genuinely authentic — correct techniques, flavor balance, dish structure, and cooking methods for that culture, using only available ingredients that fit.
4. Do NOT invent or assume any ingredients.
${prefs?`\nUser preferences: ${prefs}`:""}

Generate exactly 4 varied recipes. Return ONLY valid JSON, no markdown:
{"recipes":[{"name":"...","tagline":"max 8 words","description":"2-3 sentences including cultural context","time":"e.g. 25 mins","serves":"e.g. 4","difficulty":"Easy/Medium/Hard","ingredients":["amount + ingredient from list only"],"steps":["Step 1...","Step 2...","Step 3...","Step 4...","Step 5..."],"tip":"cooking tip for the dish"}]}`;
    try {
    ...
    } catch{setErr("The chuck wagon broke down. Try again, partner.");}
    setLoading(false);
  };

  const togList=(item,list,setList)=>setList(p=>p.includes(item)?p.filter(x=>x!==item):[...p,item]);
      ...
      <div className="app-bg">
        ...
        <div className="header-plank" style={{padding:"24px 20px",textAlign:"center"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:16,marginBottom:6}}>
            <Star s={32}/><h1 className="rye" style={{fontSize:"clamp(28px,5vw,48px)",color:"#f0c060",textShadow:"2px 3px 0 rgba(0,0,0,0.6)",letterSpacing:"0.1em"}}>Oasis</h1><Star s={32}/>
          </div>
        ...
        <div style={{maxWidth:960,margin:"0 auto",padding:"28px 16px",display:"flex",flexDirection:"column",gap:24}}>
          ...
          <div className="parchment" style={{borderRadius:4,overflow:"hidden"}}>
            <div style={{background:"#3d2008",padding:"14px 24px",borderBottom:"2px solid #8b5a1a",display:"flex",alignItems:"center",gap:12}}>
              <Star s={18}/><h2 className="rye" style={{color:"#f0c060",fontSize:18,letterSpacing:"0.06em"}}>Step 1 — What's In Yer Pantry?</h2>
            </div>
              ...
              {mode==="manual"&&(
                <div>
                  ...
                  <div style={{display:"flex",gap:8,marginBottom:14}}>
                    <input className="fi" placeholder="e.g. chicken thighs, jalapeño..." value={txt} onChange={e=>setTxt(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addTag()} style={{flex:1}}/>
                  ...
                  {tags.length>0?<div style={{display:"flex",flexWrap:"wrap",gap:6}}>{tags.map(t=><span key={t} className="itag">{t}<button onClick={()=>setTags(p=>p.filter(x=>x!==t))}>×</button></span>)}</div>:<p style={{fontSize:12,color:"#a07840",fontStyle:"italic"}}>No ingredients yet, partner.</p>}
                </div>
              )}
              {mode==="photo"&&(
                <div>
                  ...
                  {!preview?(
                    <div className={`drop-zone${drag?" dov":""}`} style={{padding:"40px 20px",textAlign:"center",borderRadius:2}} onClick={()=>fileRef.current?.click()} onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);handleFile(e.dataTransfer.files[0])}}>
                    ...
                    <div style={{display:"flex",gap:20,flexWrap:"wrap",alignItems:"flex-start"}}>
                      ...
                      <div style={{flex:1,minWidth:180}}>
                        {analyzing?<div style={{display:"flex",alignItems:"center",gap:8,color:"#8b5a1a"}}><svg width="20" height="20" viewBox="0 0 20 20" className="spin"><circle cx="10" cy="10" r="8" stroke="#c9922a" strokeWidth="2.5" fill="none" strokeDasharray="30 10"/></svg><span style={{fontStyle:"italic",fontSize:13}}>Squintin' at yer photo...</span></div>
                        :photoIngs.length>0?<><p className="sl">Spotted These</p><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{photoIngs.map(i=><span key={i} className="itag">{i}<button onClick={()=>setPhotoIngs(p=>p.filter(x=>x!==i))}>×</button></span>)}</div></>
            ...
            {ingredients.length>0&&(
              <div style={{borderTop:"1px solid #c9922a",background:"rgba(139,90,26,0.08)",padding:"10px 24px",display:"flex",alignItems:"center",gap:8}}>
                <Star s={14}/><span className="cp" style={{fontSize:12,color:"#8b5a1a",fontWeight:700}}>{ingredients.length} ingredient{ingredients.length!==1?"s":""} rounded up</span>
              </div>
            )}
          </div>
          ...
          <div className="parchment" style={{borderRadius:4,overflow:"hidden"}}>
            <div style={{background:"#3d2008",padding:"14px 24px",borderBottom:"2px solid #8b5a1a",display:"flex",alignItems:"center",gap:12}}>
              <Star s={18}/><h2 className="rye" style={{color:"#f0c060",fontSize:18,letterSpacing:"0.06em"}}>Step 2 — Whatcha Hankerin' For?</h2>
              <span style={{marginLeft:"auto",fontFamily:"Courier Prime,monospace",fontSize:11,color:"#8b5a1a"}}>(optional)</span>
            </div>
            <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:18}}>
              <div><p className="sl">Meal Type</p><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{MEAL_TYPES.map(m=><Pill key={m} label={m} active={meal===m} onClick={()=>setMeal(meal===m?"":m)}/>)}</div></div>
              <div><p className="sl">Cuisine Style</p><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{CUISINES.map(c=><Pill key={c} label={c} active={cuisines.includes(c)} onClick={()=>togList(c,cuisines,setCuisines)}/>)}</div></div>
              <div><p className="sl">Taste Profile</p><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{TASTES.map(t=><Pill key={t} label={t} active={tastes.includes(t)} onClick={()=>togList(t,tastes,setTastes)}/>)}</div></div>
              <div><p className="sl">Anything Else, Cowpoke?</p><textarea className="fta" placeholder="e.g. need to feed 8, no nuts, quick dinner..." value={notes} onChange={e=>setNotes(e.target.value)} rows={3}/></div>
            </div>
          </div>

          {err&&<div style={{background:"rgba(139,40,0,0.2)",border:"2px solid #8b2800",padding:"12px 20px",textAlign:"center"}}><p className="cp" style={{fontSize:13,color:"#c9501a"}}>⚠ {err}</p></div>}
